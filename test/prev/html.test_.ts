...
describe('html', () => {
  describe('justifyContent', () => {
    ...
    it('adds line breaks to existing text', () => {
      ...
      expect(lines).toEqual(['This is some', 'test content', 'that should be', 'wrapped']);
    });
    ...
    it('applies hyphenation', () => {
      ...
      const lines = extractLines(para);
      expect(lines).toEqual(['This is som', '-e test conte', 'nt that shoul', '-d be wrapped']);
    });
    ...
    it('justifies multiple paragraphs', () => {
      ...
      expect(lines1).toEqual(['test that', 'multiple paragraphs', 'are justified']);
